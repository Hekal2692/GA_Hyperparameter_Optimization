...
- The outer GA in [`OptimizerGa.py`](OptimizerGa.py) tunes the inner GA hyperparameters with a weighted objective built from repeated-run mean makespan and repeated-run mean runtime.
  The search space includes the main inner-GA controls (`pop_size`, `cxpb`, `mutpb`, `ngen`) and the inner mutation sub-probabilities for task order, processor allocation, message priority, and message path genes. This branch also supports a fair exact computation-budget constraint through `outer_ga.computation_budget`, for example `pop_size * ngen = 2520`, while still evolving the full inner-GA hyperparameter set.
...
- a text run log
- a plot of the outer GA objective history, with the main fitness panel showing best-so-far weighted-score progress
...
- a `*_history.json` file with generation-level metrics written on the final archive-wide weighted-objective normalization basis
...
The outer plot and history optimize the weighted outer objective, not just the best single inner-GA run.
