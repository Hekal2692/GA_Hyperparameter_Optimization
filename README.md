...
- The outer GA in [`OptimizerGa.py`](OptimizerGa.py) tunes the inner GA hyperparameters with a weighted multi-objective score built from the mean makespan and the mean runtime across repeated inner runs.
  ...
  The weights live under `outer_ga.objective_weights` in the config. The two objectives are normalized before they are combined, so runtime can influence selection even though its raw numeric scale is much smaller than makespan.
...
Run a specific instance with the current multi-objective weights:

```bash
python OptimizerGa.py example_70T.json --config ga_config.json
```

To change the outer objective tradeoff, edit `outer_ga.objective_weights` in your config before launching a run. For example:
...
Regenerate plots for one or more existing multi-objective run folders without rerunning the GA:

```bash
python OptimizerGa.py --replot-run-dir logs/multiobjectiverun_50T_20260428_141321 logs/multiobjectiverun_70T_20260428_111327
```
...
- a text run log
- a main history plot for the weighted outer objective score
...
Multi-objective run folders are named `multiobjectiverun_<instance>_<timestamp>`.

The outer plot and history optimize a weighted score based on:

- the mean makespan over `inner_repeats`
- the mean runtime over the same `inner_repeats`

The selected best configuration in `*_best_result.json` is chosen by that weighted score, not by the best single inner-GA repeat. The file still includes the best single repeat schedule from that winning configuration so it can be inspected later.
