...
def student_t_critical_975(degrees_of_freedom):
    """Return two-sided 95% Student-t critical values without importing SciPy."""
    critical_values = {
        1: 12.706,
        2: 4.303,
        3: 3.182,
    ...
    if degrees_of_freedom < 40:
        return critical_values[30]
    if degrees_of_freedom < 50:
        return critical_values[40]
    if degrees_of_freedom < 60:
        return critical_values[50]
    if degrees_of_freedom < 80:
        return critical_values[60]
    if degrees_of_freedom < 100:
        return critical_values[80]
    if degrees_of_freedom < 120:
        return critical_values[100]
    return 1.96
...
def compute_summary_statistics(values):
    ...
    if value_count > 1:
        ...
        t_critical = student_t_critical_975(value_count - 1)
...
def compute_welch_ttest(sample_a, sample_b):
    ...
    var_a = sample_variance(sample_a)
    var_b = sample_variance(sample_b)
    standard_error = math.sqrt((var_a / len(sample_a)) + (var_b / len(sample_b)))
    if standard_error == 0:
        t_statistic = 0.0
        ...
        one_sided_pvalue = normal_cdf(t_statistic)
        two_sided_pvalue = 2.0 * min(one_sided_pvalue, 1.0 - one_sided_pvalue)

    return {
        "available": True,
        "method": "welch_ttest_normal_approximation",
