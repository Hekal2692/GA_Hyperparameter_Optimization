...
HYPERPARAMETER_INDEX_BY_NAME = {
    name: index
    for index, (name, _) in enumerate(OUTER_HYPERPARAMETER_SPECS)
}
...
class OuterGAResult:
    ...
    best_makespan: float
    best_weighted_objective_score: Optional[float]
    best_avg_inner_runtime_seconds: Optional[float]
    history: List[Dict]
    evaluation_records: Optional[List[Dict]] = None
    best_evaluation_id: Optional[int] = None
    population_evaluation_snapshots: Optional[Dict[int, List[int]]] = None
    run_dir: Optional[Path] = None
    log_path: Optional[Path] = None
    plot_path: Optional[Path] = None
    history_path: Optional[Path] = None
    evaluation_log_path: Optional[Path] = None
    best_result_path: Optional[Path] = None
    analysis_plot_paths: Optional[dict] = None
...
def resolve_outer_objective_weights(outer_settings):
    """Resolve and normalize the weighted outer objective components."""
    configured_weights = outer_settings.get("objective_weights", {})
    makespan_weight = float(configured_weights.get("mean_makespan", 0.5))
    runtime_weight = float(
        configured_weights.get(
            "mean_runtime_seconds",
            configured_weights.get("mean_runtime", 0.5),
        )
    )
...
def get_active_outer_hyperparameter_names(config):
    """Return the hyperparameters that the outer GA is allowed to evolve."""
    outer_settings = get_outer_ga_settings(config)
    configured_names = outer_settings.get("evolved_hyperparameters")
    valid_names = {name for name, _ in OUTER_HYPERPARAMETER_SPECS}
    if not configured_names:
        return [name for name, _ in OUTER_HYPERPARAMETER_SPECS]
    ...
    for name in configured_names:
        if name not in valid_names:
            raise ValueError(
                f"Unknown outer hyperparameter in evolved_hyperparameters: {name}"
            )
...
def get_computation_budget_settings(config):
    """Return the configured computation-budget constraint for inner GA runs."""
    outer_settings = get_outer_ga_settings(config)
    configured = outer_settings.get("computation_budget", {}) or {}
    mode = configured.get("mode", "none")
    return {
        "mode": mode,
        "budget": configured.get("budget"),
    }


def clamp_hyperparameter_value(name, value, config):
    """Clamp one hyperparameter to the configured search-space bounds."""
    bounds = get_search_space(config)[name]
    if isinstance(bounds["min"], int) and isinstance(bounds["max"], int):
        return min(max(int(round(value)), bounds["min"]), bounds["max"])
    return min(max(float(value), bounds["min"]), bounds["max"])


def hyperparameter_dict_to_list(hyperparameters):
    return [hyperparameters[name] for name, _ in OUTER_HYPERPARAMETER_SPECS]


def resolve_default_inner_hyperparameter_value(name, config):
    """Map inactive outer genes to the fixed inner-GA defaults used on this branch."""
    if name in {"pop_size", "cxpb", "mutpb", "ngen"}:
        return get_static_scheduler_ga_settings(config)[name]
    return config["scheduler"]["mutation"][name]


def build_default_outer_hyperparameters(config):
    """Build one full hyperparameter dictionary using the branch's fixed defaults."""
    hyperparameters = {}
    for name, value_type in OUTER_HYPERPARAMETER_SPECS:
        value = resolve_default_inner_hyperparameter_value(name, config)
        hyperparameters[name] = int(round(value)) if value_type is int else float(value)
    return hyperparameters


def build_feasible_budget_pairs(config):
    """Enumerate exact fair (pop_size, ngen) pairs inside the configured bounds."""
    budget_settings = get_computation_budget_settings(config)
    if budget_settings["mode"] != "exact_product":
        return []

    budget = budget_settings.get("budget")
    if budget is None:
        raise ValueError("computation_budget.budget must be set when mode=exact_product.")

    pop_bounds = get_search_space(config)["pop_size"]
    ngen_bounds = get_search_space(config)["ngen"]
    feasible_pairs = []
    for pop_size in range(pop_bounds["min"], pop_bounds["max"] + 1):
        if budget % pop_size != 0:
            continue
        ngen = budget // pop_size
        if ngen_bounds["min"] <= ngen <= ngen_bounds["max"]:
            feasible_pairs.append((pop_size, ngen))

    if not feasible_pairs:
        raise ValueError(
            "No feasible exact (pop_size, ngen) pairs satisfy the configured "
            "computation budget inside the current search-space bounds."
        )
    return feasible_pairs


def choose_nearest_budget_pair(pop_size, ngen, feasible_pairs, config):
    """Snap a candidate to the closest exact fair (pop_size, ngen) pair."""
    pop_bounds = get_search_space(config)["pop_size"]
    ngen_bounds = get_search_space(config)["ngen"]
    pop_span = max(1, pop_bounds["max"] - pop_bounds["min"])
    ngen_span = max(1, ngen_bounds["max"] - ngen_bounds["min"])
    return min(
        feasible_pairs,
        key=lambda pair: (
            ((pair[0] - pop_size) / pop_span) ** 2
            + ((pair[1] - ngen) / ngen_span) ** 2,
            pair[0],
            pair[1],
        ),
    )
...
def enforce_computation_budget(hyperparameters, config):
    """Enforce the configured computation-budget rule on pop_size and ngen."""
    budget_settings = get_computation_budget_settings(config)
    if budget_settings["mode"] != "exact_product":
        return hyperparameters

    feasible_pairs = build_feasible_budget_pairs(config)
    selected_pop_size, selected_ngen = choose_nearest_budget_pair(
        hyperparameters["pop_size"],
        hyperparameters["ngen"],
        feasible_pairs,
        config,
    )
    hyperparameters["pop_size"] = selected_pop_size
    hyperparameters["ngen"] = selected_ngen
    return hyperparameters


def resolve_outer_hyperparameters(individual, config):
    """Convert one live individual into a fully resolved hyperparameter dictionary."""
    active_names = set(get_active_outer_hyperparameter_names(config))
    hyperparameters = build_default_outer_hyperparameters(config)

    for index, (name, value_type) in enumerate(OUTER_HYPERPARAMETER_SPECS):
        if name not in active_names:
            continue
        raw_value = individual[index]
        if value_type is int:
            hyperparameters[name] = clamp_hyperparameter_value(name, raw_value, config)
        else:
            hyperparameters[name] = clamp_hyperparameter_value(name, float(raw_value), config)

    return enforce_computation_budget(hyperparameters, config)
...
def normalize_hyperparameter_types(individual, config):
    ...
    return hyperparameter_dict_to_list(resolve_outer_hyperparameters(individual, config))


def individual_to_dict(individual, config):
    """Convert an outer individual to a readable hyperparameter dictionary."""
    hyperparameters = resolve_outer_hyperparameters(individual, config)
    return {
        name: value if value_type is int else round(value, 3)
        for (name, value_type), value in zip(
            OUTER_HYPERPARAMETER_SPECS,
            hyperparameter_dict_to_list(hyperparameters),
        )
    }
...
def create_run_output_paths(am_name, config, log_dir=None):
    ...
    run_name = f"computation_cost_{safe_am_name}_{timestamp}"
    ...
    return {
        ...
        "best_individual_survival_plot_path": run_dir / f"{run_name}_best_individual_survival.png",
    ...
    for index, individual in enumerate(population, start=1):
        if individual.fitness.valid:
            status = (
                ...
                f", weighted_score={getattr(individual, 'weighted_objective_score', individual.fitness.values[0])}"
                f", mean_makespan={getattr(individual, 'mean_makespan', 'unknown')}"
...
def create_hyperparameter_individual(config):
    ...
    if budget_settings["mode"] == "exact_product" and (
        "pop_size" in active_names or "ngen" in active_names
    ):
        pop_size, ngen = random.choice(build_feasible_budget_pairs(config))
        hyperparameters["pop_size"] = pop_size
        hyperparameters["ngen"] = ngen

    for name, value_type in OUTER_HYPERPARAMETER_SPECS:
        if name not in active_names:
            continue
        if budget_settings["mode"] == "exact_product" and name in {"pop_size", "ngen"}:
            continue

        bounds = search_space[name]
        if value_type is int:
            hyperparameters[name] = random.randint(bounds["min"], bounds["max"])
        else:
            hyperparameters[name] = random.uniform(bounds["min"], bounds["max"])

    return normalize_hyperparameter_types(
        hyperparameter_dict_to_list(hyperparameters),
        config,
    )
...
def mutate_hyperparameters(individual, config):
    ...
    active_names = set(get_active_outer_hyperparameter_names(config))
    budget_settings = get_computation_budget_settings(config)
    budget_pair_mutated = False
    for index, (name, value_type) in enumerate(OUTER_HYPERPARAMETER_SPECS):
        if name not in active_names:
            continue

        if budget_settings["mode"] == "exact_product" and name in {"pop_size", "ngen"}:
            if budget_pair_mutated or random.random() >= 0.5:
                continue
            pop_size, ngen = random.choice(build_feasible_budget_pairs(config))
            individual[HYPERPARAMETER_INDEX_BY_NAME["pop_size"]] = pop_size
            individual[HYPERPARAMETER_INDEX_BY_NAME["ngen"]] = ngen
            budget_pair_mutated = True
            continue
...
def crossover_hyperparameters(ind1, ind2, config):
    ...
    if budget_settings["mode"] == "exact_product" and (
        ...
        if random.random() < 0.5:
            pop_index = HYPERPARAMETER_INDEX_BY_NAME["pop_size"]
            ngen_index = HYPERPARAMETER_INDEX_BY_NAME["ngen"]
            ind1[pop_index], ind2[pop_index] = ind2[pop_index], ind1[pop_index]
            ind1[ngen_index], ind2[ngen_index] = ind2[ngen_index], ind1[ngen_index]

    for index in range(len(ind1)):
        name = OUTER_HYPERPARAMETER_SPECS[index][0]
        if name not in active_names:
            continue
        if budget_settings["mode"] == "exact_product" and name in {"pop_size", "ngen"}:
            continue
...
def individual_mean_makespan(individual):
    return getattr(individual, "mean_makespan", None)
...
def is_better_outer_candidate(candidate, incumbent):
    return outer_candidate_sort_key_from_individual(
        candidate
    ) < outer_candidate_sort_key_from_individual(incumbent)
...
def build_generation_history_row(
    ...
    runtimes = [individual_avg_inner_runtime_seconds(individual) for individual in population]
    generation_best_weighted_objective_score = individual_weighted_objective_score(
        generation_best
    )
    generation_avg_weighted_objective_score = mean(weighted_scores)
    generation_worst_weighted_objective_score = max(weighted_scores)
...
def format_hyperparameters_for_plot(hyperparameters):
    ...
    for key, label in (
        ...
        if key in hyperparameters:
            primary_parts.append(f"{label}={hyperparameters[key]}")

    mutation_parts = []
    for key, label in (
        ("task_order_probability", "task_mut"),
        ("processor_allocation_probability", "proc_mut"),
        ("message_priority_shuffle_probability", "prio_mut"),
        ("message_path_index_probability", "path_mut"),
    ):
        if key in hyperparameters:
            mutation_parts.append(f"{label}={hyperparameters[key]}")

    lines = []
    if primary_parts:
        lines.append(", ".join(primary_parts))
    if mutation_parts:
        lines.append(", ".join(mutation_parts))
    return "\n".join(lines) if lines else "No hyperparameters recorded"
...
def format_hyperparameters_for_summary(hyperparameters):
    ...
    mutation_parts = []
    for key, label in (
        ("task_order_probability", "task"),
        ("processor_allocation_probability", "proc"),
        ("message_priority_shuffle_probability", "prio"),
        ("message_path_index_probability", "path"),
    ):
        if key in hyperparameters:
            mutation_parts.append(f"{label}={hyperparameters[key]}")
...
def validate_top_candidates_on_unseen_seeds(
    ...
    for rank, candidate_record in enumerate(candidate_records, start=1):
        ...
        result = {
            ...
            "training_evaluation_id": candidate_record["evaluation_id"],
            "hyperparameters": hyperparameters,
            "training_weighted_objective_score": candidate_record.get(
                "weighted_objective_score"
            ),
...
def outer_ga(
    ...
    def evaluate_outer(individual, generation, population_index, stage):
        ...
        evaluation_record = {
            ...
            "stage": stage,
            "hyperparameters": hyperparameters,
            "computation_budget_mode": budget_settings["mode"],
            "target_computation_budget": budget_settings.get("budget"),
            "actual_computation_budget": pop_size * ngen,
        ...
        individual.fitness.values = (avg_makespan,)
    ...
    for generation in range(1, outer_ngen + 1):
        ...
        elites = [
            ...
            for individual in sorted(population, key=outer_candidate_sort_key_from_individual)[
                :elite_count
            ]
        ]
        ...
        print(
            "Generation best weighted objective score:",
            row["generation_best_weighted_objective_score"],
        )
        ...
        print(
            "Best weighted objective score so far:",
            row["best_so_far_weighted_objective_score"],
        )
...
def plot_outer_ga_history(history, save_path="outer_ga_hyperparameter_search.png", show_plot=False):
    ...
    if uses_final_archive_normalization:
        fitness_axis.set_title("Weighted fitness progress (best-so-far, final-normalized)")
    else:
        fitness_axis.set_title("Weighted fitness progress (best-so-far)")
...
def plot_population_objective_std(history, save_path, show_plot=False):
    ...
    if uses_weighted_objective:
        axis.set_ylabel("Std of weighted objective score")
...
def run_default_outer_ga(
    ...
    evolved_hyperparameters = get_active_outer_hyperparameter_names(config)
    budget_settings = get_computation_budget_settings(config)
    feasible_budget_pairs = build_feasible_budget_pairs(config) if (
        budget_settings["mode"] == "exact_product"
    ) else []
    random_immigrant_count = resolve_random_immigrant_count(
        outer_pop_size,
        elite_count,
    ...
    with log_path.open("w", encoding="utf-8", buffering=1) as log_file, redirect_stdout(log_file):
        ...
        print("Population origin plot:", population_origin_plot_path.resolve())
        print("Best-individual survival plot:", best_individual_survival_plot_path.resolve())
        ...
        print(
            "Outer GA settings:",
            {
                ...
                "objective_weights": objective_weights,
                "evolved_hyperparameters": evolved_hyperparameters,
                "computation_budget": budget_settings,
            },
        )
        if feasible_budget_pairs:
            print("Feasible exact fair (pop_size, ngen) pairs:", feasible_budget_pairs)
        print(
            "Analysis settings:",
            {
        ...
        plot_history, analysis_plot_paths = generate_outer_ga_analysis_plots(
            ...
            output_paths={
                ...
                "validation_boxplot_path": validation_boxplot_path,
        ...
        write_json(history_path, plot_history)
        ...
        validation_payload = {
            "description": (
                ...
                "Training candidates are ranked by the weighted outer objective score."
            ),
            "top_k_validation_candidates": top_k_validation_candidates,
            "validation_repeats": validation_repeats,
            "validation_seed_offset": validation_seed_offset,
    ...
    print("Final best weighted objective score:", result.best_weighted_objective_score)
