...
def parse_args(argv=None):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Fixed inner-GA crossover probabilities to sweep across runs. "
            "These are not part of the outer-GA individual."
        ),
    )
    parser.add_argument(
        "--fixed-mutpb-values",
        ...
        default=None,
        help=(
            "Fixed inner-GA mutation probabilities to sweep across runs. "
            "These are not part of the outer-GA individual."
        ),
    )
    parser.add_argument(
        ...
        default=None,
        help=(
            "Shared fixed scheduler-mutation probabilities to apply to all 4 scheduler "
            "mutation genes in each run. These are not part of the outer-GA individual."
        ),
    )
...
def resolve_fixed_parameter_grid(
    config,
    *,
    fixed_cxpb_values=None,
    fixed_mutpb_values=None,
    ...
    default_shared_mutation = float(
        scheduler_mutation["task_order_probability"]
    )

    cxpb_values = (
        [default_cxpb]
        if not fixed_cxpb_values
        else [validate_probability_value("fixed cxpb", value) for value in fixed_cxpb_values]
    )
    mutpb_values = (
        [default_mutpb]
        if not fixed_mutpb_values
        else [validate_probability_value("fixed mutpb", value) for value in fixed_mutpb_values]
    )
    shared_values = (
...
def fixed_parameter_combo_label(parameter_combo):
    return (
        f"cxpb={parameter_combo['fixed_cxpb']:.2f}, "
        f"mutpb={parameter_combo['fixed_mutpb']:.2f}, "
        f"shared_mut={parameter_combo['shared_scheduler_mutation_probability']:.2f}"
    )
    ...
    for input_json_path in input_json_paths:
        ...
        for budget_row in budget_metadata_rows:
            for parameter_combo in fixed_parameter_grid:
                combo_label = fixed_parameter_combo_label(parameter_combo)
                ...
                for outer_seed in outer_seeds:
                    ...
                    task_dir_name = (
                        ...
                        f"__seed_{safe_path_component(outer_seed)}"
                    )
                    ...
                    tasks.append(
                        {
                            ...
                            "fixed_parameter_combo_label": combo_label,
...
def build_run_record(
    ...
    *,
    ...
    return {
        ...
        "outer_random_seed": outer_seed,
        "fixed_cxpb": result.best_hyperparameters["cxpb"],
        "fixed_mutpb": result.best_hyperparameters["mutpb"],
        "shared_scheduler_mutation_probability": result.best_hyperparameters[
            "task_order_probability"
        ],
        "fixed_parameter_combo_label": (
            f"cxpb={result.best_hyperparameters['cxpb']:.2f}, "
            f"mutpb={result.best_hyperparameters['mutpb']:.2f}, "
            f"shared_mut={result.best_hyperparameters['task_order_probability']:.2f}"
        ),
...
def sort_run_record_key(record):
    return (
        ...
        record["budget"],
        record.get("fixed_cxpb"),
        record.get("fixed_mutpb"),
        record.get("shared_scheduler_mutation_probability"),
        ...
        rows.append(
            {
                ...
                "fixed_parameter_combo_label": records[0]["fixed_parameter_combo_label"],
...
def build_normalized_budget_summary(budget_instance_rows):
    ...
    for row in budget_instance_rows:
        ...
        minimum_key = (
            ...
            row["fixed_cxpb"],
            row["fixed_mutpb"],
            row["shared_scheduler_mutation_probability"],
        )
        previous = per_instance_minima.get(minimum_key)
        if previous is None or metric < previous:
            per_instance_minima[minimum_key] = metric
    ...
    for row in budget_instance_rows:
        ...
        instance_minimum = per_instance_minima.get(minimum_key)
        metric = row.get("selected_validation_mean_makespan_mean")
        ...
        grouped.setdefault(
            (
                ...
                row["fixed_mutpb"],
                row["shared_scheduler_mutation_probability"],
                row["fixed_parameter_combo_label"],
            ),
            [],
        ).append(metric / instance_minimum)
    ...
    for (
        ...
        shared_scheduler_mutation_probability,
        combo_label,
    ), normalized_values in sorted(grouped.items()):
        rows.append(
            {
                "budget": budget,
                "fixed_cxpb": fixed_cxpb,
...
def plot_normalized_budget_summary(
    ...
    *,
    ...
    for row in normalized_budget_rows:
        grouped.setdefault(row["fixed_parameter_combo_label"], []).append(row)

    for combo_label, rows in sorted(grouped.items()):
        sorted_rows = sorted(rows, key=lambda row: row["budget"])
        budgets = [row["budget"] for row in sorted_rows]
        normalized_means = [
